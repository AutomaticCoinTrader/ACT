...
func (t TradeCommonResponse) needRetry() (bool) {
	if t.Success == 0 {
		...
		if t.Error == "order is too new" {
			return false
		} else if t.Error == "order not found" {
			return false
...
func (t *TradeParams) fixupPriceAndAmount(r *Requester) {
	...
	fixedPrice := math.Floor((float64(int64((t.Price/priceUnit) + 0.00000000001))*priceUnit)*10000000000) / 10000000000
	if fixedPrice != t.Price {
		t.Price = fixedPrice
	}
	fixedAmount := math.Floor((float64(int64((t.Amount/amountUnit) + 0.0000001))*amountUnit)*1000000) / 1000000
	...
	for {
		...
		if err != nil || newRes.(*TradeResponse).needRetry() {
			...
			if err != nil {
				...
				errMsg = newRes.(*TradeResponse).Error
			}
			retry := retryCallback(&tradeParams.Price, &tradeParams.Amount, errMsg, retryCallbackData)
			if !retry {
				if err != nil {
